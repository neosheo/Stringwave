...
postgres_uri = f"postgresql+psycopg2://{postgres_user}:{postgres_pass}@postgres:5432"
...
app.config["SQLALCHEMY_BINDS"] = {
    ...
    "discogs": f"{postgres_uri}/cogmera",
    "main": f"{postgres_uri}/stringwave",
}
...
def create_admin_user(bcrypt_obj):
    ...
    hashed_pw = bcrypt_obj.generate_password_hash(password).decode("utf-8")
    with db.session.begin():
        admin_user_exists = db.session.query(Users).filter(Users.user_id == 1).first()
